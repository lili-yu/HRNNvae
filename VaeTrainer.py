...
class Statistics(object):
    ...
    """
    def __init__(self, loss=0, kldloss=0.0, n_words=0, n_correct=0):
        self.loss = loss
    ...
    def output(self, epoch, batch, n_batches, start):
        t = self.elapsed_time()
        #print(type(self.kldloss))
        #print(type(self.loss))
        print(("Epoch %2d, %5d/%5d; loss: %6.2f; kldloss: %6.2f; acc: %6.2f; ppl: %6.2f; " +
              ...
              (epoch, batch,  n_batches,
               ...
               self.n_words / (t + 1e-5),
               time.time() - start))
        sys.stdout.flush() # kldloss: %6.2f;

    def log(self, prefix, experiment, optim):
        t = self.elapsed_time()
        experiment.add_scalar_value(prefix + "_loss", self.loss)
        experiment.add_scalar_value(prefix + "_kldloss", self.kldloss)
...
class VaeTrainer(object):
    ...
    def train(self, epoch, report_func=None):
        ...
        for i, batch in enumerate(self.train_iter):  ### enumberate....
                ...
                for j in range(0, target_size-1, trunc_size):
                    ...
                    #   j, trunc_size, self.shard_size)

                    target = batch.tgt[j + 1: j + trunc_size ]
                    
                    loss, stats = self.train_loss.compute_loss(batch, outputs, target, mu, logvar)
    ...
    def validate(self):
        ...
        for batch in self.valid_iter:
            ...
            outputs, dec_state, mu, logvar= self.model(src, tgt, src_lengths)

            # Compute loss.
            gen_state = onmt.Loss.make_gen_state(
                outputs, batch,  mu, logvar, (0, batch.tgt.size(0)))
